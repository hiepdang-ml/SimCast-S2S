...
class UNetDenoiserShapeTest(unittest.TestCase):
    def test_unet_denoiser_output_shape(self) -> None:
        ...
        model = UNetDenoiser(
            ...
            transformer_feedforward_dim=8,
            n_conv_layers_per_scaling_block=2,
            n_transformer_encoder_layers_per_scaling_block=3,
            n_transformer_decoder_layers_per_scaling_block=3,
            n_conv_layers_per_mid_block=2,
            n_transformer_encoder_layers_per_mid_block=3,
            n_transformer_decoder_layers_per_mid_block=3,
            n_attention_heads=1,
            switch_ratio=0.0,
        )
        model.eval()
        ...
        batch_size = 2
        target = torch.randn(batch_size, 16, 24, 36)
        condition_mu = torch.randn(batch_size, 28, 36)
        condition_logvar = torch.randn(batch_size, 28, 36)
        step = torch.zeros(batch_size, 1, dtype=torch.long)
        days = torch.arange(3, dtype=torch.long).repeat(batch_size, 1)
